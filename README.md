...
> ⚠️ **需要先安装 Desktop Mate **。本工具是其 mod，不是独立闹钟，你可以通过Desktop Mate演示安装包.exe安装下载。  
> **项目目的**：将 Desktop Mate 的闹钟特效触发逻辑分离为独立组件，使得无需等待系统时钟匹配、无需操作游戏 UI，一行命令即可触发角色播放闹钟动画和音效。可用于桌面提醒、自动化工作流等场景。~~谁能拒绝Miku的knockknock闹钟提醒呢~~

可以转化成一条命令，桌面角色立刻播放闹钟特效。
